...
movieapp.service('dataService', function() {
    ...
    var friendRequests = JSON.parse(localStorage.getItem('friendRequests')) || [];
  ...
  this.acceptRequest = function(request) {
        // Get the friend's booking details if they exist
        let friendData = JSON.parse(localStorage.getItem(request.username));
        if (friendData && friendData.bookings) {
          request.bookings = friendData.bookings;
        } else {
          request.bookings = [];
        }
...
movieapp.controller("registerController", function ($scope,dataService) {
  ...
  $scope.register = function () {
    if ($scope.registerForm.$valid) {
      localStorage.setItem($scope.user.username, JSON.stringify($scope.user));
       ...
       // Add user to the friend request list for other users
       dataService.addFriendRequest($scope.user);
...
movieapp.controller("userController", function ($scope, DataService, dataService,$location) {
  ...
  $scope.showFriendBookings = function(friend) {
